<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if(is_blank($user['cnf_password'])){
      $errors[] = "Password confirmation cannot be left blank!";
    }
    ...
    if(!preg_match('/[A-Z]+[a-z]+[-_@.]+/',$user['password'])){
      $errors[] = "Password must contain an uppercase letter, a lower case letter and a symbol!";
    }
  ...
  function throttle_time($username) {
    ...
    if($remaining_lockout < 0) {
      ...
      return "Too many failed logins for this username. You will need to wait " . ceil($remaining_lockout/60) . " minutes before attempting another login.";
    }
  }
